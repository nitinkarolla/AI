...
class NeuralNetwork():
    ...
    def _backward(self, x, y, out):
        ...
        for curr_layer in layers_reversed:
            ...
            # For the other layers use the derivatives from previous layers
            ...
            sigma_derivative = activation_derivative(self.weights[next_layer] @ current_layer_output)
            ...
            for curr_layer_neuron in range(self.num_neurons_each_layer[curr_layer]):
                ...
                next_layer_weights_without_bias = self.weights[next_layer][:, 1:]
                second_term = sigma_derivative * next_layer_weights_without_bias[:, curr_layer_neuron]
        ...
        for curr_layer in layers_reversed:
            ...
            for curr_layer_neuron in range(self.num_neurons_each_layer[curr_layer]):
                first_term = output_derivatives[curr_layer][curr_layer_neuron]
                sigma_derivative = activation_derivative(self.weights[curr_layer][curr_layer_neuron] @ previous_layer_output)
                second_term = sigma_derivative * previous_layer_output
    ...
    def _forward(self, x):
        ...
        for curr_layer in self.layers:
            ...
            weights = self.weights[curr_layer]
            activation_for_this_layer = self.activations[curr_layer]
            activation_function = self.activations_functions[activation_for_this_layer]
            if curr_layer == 0:
                out[curr_layer] = activation_function(weights @ x)
            else:
                previous_layer_output = out[curr_layer - 1].copy()
                previous_layer_output = np.insert(previous_layer_output, obj = 0, values = 1)
                out[curr_layer] = activation_function(weights @ previous_layer_output)
        return out
...
nn = NeuralNetwork(num_hidden_layers = 4, num_neurons_each_layer = [2, 3, 3, 4], epochs = 10000)
nn.fit(X = x, y = [1, 0, 1, 0, 0])
print(nn.weights)
print(nn.predict(x))
